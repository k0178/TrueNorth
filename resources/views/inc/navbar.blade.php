
{{-- <div class="d-flex align-items-center " style="background: white; border-bottom:1px #f0eeee solid;">
    <div class="">
      <a class="navbar-brand ps-3" href="/home">   <img src="/img/tnglogo.png" alt="" class="" width="213px" height= "73px" ></a>
    </div>
<div class="w-100">
    <nav class="navbar text-center navbar-expand-lg ps-4" style=" background-color: white;">
<div class="collapse navbar-collapse align-items-center" id="navbarSupportedContent">
  <ul class="navbar-nav mr-auto">
    <li class="nav-item dropdown">
      <a class="nav-link me-3"  style="font-size: 14px;" href="/store" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        PRE-LOVED
      </a>
      <div class="dropdown-menu py-3 w-100" style="border-radius: 0%;" aria-labelledby="navbarDropdownMenuLink">
        <div class = "row px-2">
          <div class = "col">
            <a><b>Men</b></a> 
                <a class="dropdown-item" href="/store/PreLoved/men/tops">Tees</a>
                <a class="dropdown-item" href="/store/PreLoved/men/bottoms">Pants</a>
                <a class="dropdown-item" href="/store/PreLoved/men/shorts">Shorts</a>
          </div>
          <div class = "col">
            <a><b>Women</b></a> 
                <a class="dropdown-item" href="/store/PreLoved/women/tops">Tees</a>
                <a class="dropdown-item" href="/store/PreLoved/women/bottoms">Pants</a>
                <a class="dropdown-item" href="/store/PreLoved/women/shorts">Shorts</a>
          </div>
        </div>
      </div>
    </li>

    <li class="nav-item dropdown">
      <a class="nav-link me-3" style="font-size: 14px;" href="/store" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        BRAND-NEW
      </a>
      <div class="dropdown-menu" style="border-radius: 0%;" aria-labelledby="navbarDropdownMenuLink">
        <div class = "row px-2">
          <div class = col-sm-12>
            <a><b>Men</b></a> 
                <a class="dropdown-item" href="/store/BrandNew/men/tops">Tees</a>
                <a class="dropdown-item" href="/store/BrandNew/men/bottoms">Pants</a>
                <a class="dropdown-item" href="/store/BrandNew/men/shorts">Shorts</a>
          </div>
          <div class = "col">
            <a><b>Women</b></a> 
                <a class="dropdown-item" href="/store/BrandNew/women/tops">Tees</a>
                <a class="dropdown-item" href="/store/BrandNew/women/bottoms">Pants</a>
                <a class="dropdown-item" href="/store/BrandNew/women/shorts">Shorts</a>
          </div>
        </div>
      </div>
    </li>

    <li class="nav-item dropdown">
      <a class="nav-link" href="/store" style="font-size: 14px;" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        BULK
      </a>
      <div class="dropdown-menu" style="border-radius: 0%;" aria-labelledby="navbarDropdownMenuLink">
        <div class = "row px-2">
          <div class = col-sm-12>
            <a><b>Men</b></a> 
                <a class="dropdown-item" href="/store/Bulk/men/tops">Tees</a>
                <a class="dropdown-item" href="/store/Bulk/men/bottoms">Pants</a>
                <a class="dropdown-item" href="/store/Bulk/men/shorts">Shorts</a>
          </div>
          <div class = "col">
            <a><b>Women</b></a> 
                <a class="dropdown-item" href="/store/Bulk/women/tops">Tees</a>
                <a class="dropdown-item" href="/store/Bulk/women/bottoms">Pants</a>
                <a class="dropdown-item" href="/store/Bulk/women/shorts">Shorts</a>
          </div>
        </div>
      </div>
    </li>
  </ul>
</div>
<form type="get" action="{{url ('/search')}}" class="d-flex  justify-content-center align-items-center" style="float: left;">
    <input type="search" name="search" style="background: none;  " class="ps-1 search form-control" placeholder="Search for Auctions">
    <button type = "submit" class="btn me-3"><i class="bi bi-search"></i></button>
</form>
</nav>
  </div>
</div>


 --}}



  <nav class="navbar navbar-expand-lg  ">
    <div class="container-fluid text-white">
      <img src="/img/tnglogo.png" alt="" width="90px" height="30px"><button class="btn ms-1 fw-bold " type="button" style="font-size:26px; margin-right: 90px;"  onclick="location.href='/home'"> TRUE NORTH AUCTION </button>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          <li class="nav-item dropdown me-5">
            <div class="nav-link" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
              PRE-LOVED
            </div>
            <ul class="dropdown-menu">
              <a><b>Men</b></a> 
              <li><a class="drop-item" href="/store/PreLoved/men/tops">Tees</a></li>
              <li><a class="drop-item" href="/store/PreLoved/men/bottoms">Pants</a></li>
              <li><a class="drop-item" href="/store/PreLoved/men/shorts">Shorts</a></li>
              <hr>
              <a><b>Women</b></a> 
              <li><a class="drop-item" href="/store/PreLoved/women/tops">Tees</a></li>
              <li><a class="drop-item" href="/store/PreLoved/women/bottoms">Pants</a></li>
              <li> <a class="drop-item" href="/store/PreLoved/women/shorts">Shorts</a></li>
            </ul>
          </li>
          
          <li class="nav-item dropdown me-5">
            <div class="nav-link" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
              BRAND NEW
            </div>
            <ul class="dropdown-menu">
              <a><b>Men</b></a> 
              <li><a class="dropdown-item" href="/store/BrandNew/men/tops">Tees</a></li>
              <li><a class="dropdown-item" href="/store/BrandNew/men/bottoms">Pants</a></li>
              <li><a class="dropdown-item" href="/store/BrandNew/men/shorts">Shorts</a></li>
              <hr>
              <a><b>Women</b></a> 
              <li><a class="dropdown-item" href="/store/BrandNew/women/tops">Tees</a></li>
              <li><a class="dropdown-item" href="/store/BrandNew/women/bottoms">Pants</a></li>
              <li> <a class="dropdown-item" href="/store/BrandNew/women/shorts">Shorts</a></li>
            </ul>
          </li>
          <li class="nav-item dropdown">
            <div class="nav-link" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
              BULK
            </div>
            <ul class="dropdown-menu">
              <a><b>Men</b></a> 
              <li><a class="dropdown-item" href="/store/Bulk/men/tops">Tees</a></li>
              <li><a class="dropdown-item" href="/store/Bulk/men/bottoms">Pants</a></li>
              <li><a class="dropdown-item" href="/store/Bulk/men/shorts">Shorts</a></li>
              <hr>
              <a><b>Women</b></a> 
              <li><a class="dropdown-item" href="/store/Bulk/women/tops">Tees</a></li>
              <li><a class="dropdown-item" href="/store/Bulk/women/bottoms">Pants</a></li>
              <li> <a class="dropdown-item" href="/store/Bulk/women/shorts">Shorts</a></li>
            </ul>
          </li>
        </ul>
        <form class="" role="search" form type="get" action="{{url ('/search')}}">
          {{-- <input type="search" name="search" style="background: none;  " class="ps-1 search form-control" placeholder="Search for Auctions"> --}}
          <input class="form-control me-3"  name="search" type="search" placeholder="Search for Auctions " aria-label="Search">
          {{-- <button class="btn btn-outline-success" type="submit">Search</button> --}}
        </form>
    </div>
  </div>
</nav>
